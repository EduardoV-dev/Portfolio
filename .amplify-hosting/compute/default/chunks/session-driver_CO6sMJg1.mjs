!function(){try{var e="undefined"!=typeof window?window:"undefined"!=typeof global?global:"undefined"!=typeof globalThis?globalThis:"undefined"!=typeof self?self:{};e.SENTRY_RELEASE={id:"51a973bd64882fbe40a2b1fa1d425937ebce5721"};var n=(new e.Error).stack;n&&(e._sentryDebugIds=e._sentryDebugIds||{},e._sentryDebugIds[n]="78cba7d4-1364-4abc-a40b-713b292a83f3",e._sentryDebugIdIdentifier="sentry-dbid-78cba7d4-1364-4abc-a40b-713b292a83f3");}catch(e){}}();import nodeCrypto, { createHash } from 'node:crypto';
...
var sealData = createSealData(crypto$1);
var unsealData = createUnsealData(crypto$1);

const SESSION_STORE_KEY = '__amplify_session_store__';
const processAny = process;
const sessionStore = (processAny[SESSION_STORE_KEY] ??=
    new Map());
 ...
 * ```
 */
function createCookieSessionDriver(options = {}) {
    ...
    if (!config.password) {
        ...
        config.password = createHash('sha256')
            ...
            .digest('hex');
        console.warn('\x1b[33m[amplify-astro-adapter] Using auto-generated session password for development.\n' +
    ...
    return {
        ...
        async hasItem(sessionId) {
            return sessionStore.has(sessionId);
        },
        async getItem(sessionId) {
            const entry = sessionStore.get(sessionId);
            if (!entry)
                return null;
            try {
                // Decrypt the sealed data
                const decrypted = await unsealData(entry.data, {
                ...
                console.error('[session-driver] Failed to unseal session data:', err instanceof Error ? err.message : String(err));
                return null;
            }
        },
        async setItem(sessionId, value) {
            // Encrypt the data
            const dataToSeal = { _d: value };
...
export { cleanupSessionStore, createCookieSessionDriver, createCookieSessionDriver as default, generateSessionPassword, getDriverConfig, sessionStore };
